...
from scripts.validate_early_warning_inputs import main as validate_main, validate_inputs


class EarlyWarningValidationTests(unittest.TestCase):
    ...
    def _write_raw(self, path, session_id="s1", boundary_pressure=0.2, tokens=5):
        path.write_text(
            json.dumps(
                {
                    ...
                    "turns": [
                        {
                            ...
                            "summary": {"boundary_pressure": boundary_pressure},
                        }
                    ],
                }
            ),
    ...
    def test_valid_inputs_pass(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_raw(raw_neg, session_id="s_neg")
            self._write_raw(raw_pos, session_id="s_pos")
    ...
    def test_split_train_and_test_need_both_classes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_raw(raw_train_neg, session_id="train_neg")
            self._write_raw(raw_train_pos, session_id="train_pos")
            self._write_raw(raw_test_neg, session_id="test_neg")
            self._write_raw(raw_test_pos, session_id="test_pos")
    ...
    def test_cli_writes_report(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            old_argv = sys.argv
            try:
                sys.argv = ["validate_early_warning_inputs.py", "--labels", str(labels), "--inputs", str(raw_neg), str(raw_pos), "--output-dir", str(out)]
                self.assertEqual(validate_main(), 0)
            finally:
                sys.argv = old_argv
            self.assertTrue((out / "early_warning_input_validation.json").exists())
            self.assertTrue((out / "early_warning_input_validation.md").exists())
