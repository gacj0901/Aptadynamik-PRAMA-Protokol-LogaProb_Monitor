#!/usr/bin/env python
"""Validate labels.csv and PRAMA raw.json files before early-warning evaluation."""
...
from aptadynamik.evaluation.early_warning_metrics import BASELINE_SCORE_FIELDS, PRIMARY_PRAMA_SCORE, PRAMA_SCORE_FIELDS
from scripts.evaluate_early_warning import REQUIRED_LABEL_FIELDS
...
def write_markdown(path: Path, report: dict[str, Any]) -> None:
    lines = [
        "# Early-Warning Input Validation Report",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("results/early_warning_validation"))
    args = parser.parse_args()

    report = validate_inputs(args.labels, args.inputs, args.primary_score)
    args.output_dir.mkdir(parents=True, exist_ok=True)
    json_path = args.output_dir / "early_warning_input_validation.json"
    md_path = args.output_dir / "early_warning_input_validation.md"
